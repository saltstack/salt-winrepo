adobereader-xi:
  11.0.10:
    full_name: 'Adobe Reader XI (11.0.10)'
    reboot: False
    locale: en_US
    installer: 'http://ardownload.adobe.com/pub/adobe/reader/win/11.x/11.0.10/en_US/AdbeRdr11010_en_US.exe'
    install_flags: '/msi EULA_ACCEPT=YES REMOVE_PREVIOUS=YES /qn /norestart'
    uninstaller: 'http://ardownload.adobe.com/pub/adobe/reader/win/11.x/11.0.10/en_US/AdbeRdr11010_en_US.exe'
    uninstall_flags: '/qn /norestart'
  11.0.6:
    full_name: 'Adobe Reader XI (11.0.06)'
    reboot: False
    locale: en_US
    installer: 'http://ardownload.adobe.com/pub/adobe/reader/win/11.x/11.0.06/en_US/AdbeRdr11006_en_US.exe'
    install_flags: '/msi EULA_ACCEPT=YES REMOVE_PREVIOUS=YES /qn /norestart'
    uninstaller: 'http://ardownload.adobe.com/pub/adobe/reader/win/11.x/11.0.06/en_US/AdbeRdr11006_en_US.exe'
    uninstall_flags: '/qn /norestart'
