ccleaner:
  '5.19':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup519.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.18':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup518.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.17':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup517.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.16':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup516.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.15':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup515.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.14':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup514.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.13':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup513.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.12':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup512.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.11':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup511.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.10':
    full_name: 'CCleaner'
    installer: 'http://download.piriform.com/ccsetup510.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.09':
    full_name: 'CCleaner 5.09'
    installer: 'http://download.piriform.com/ccsetup509.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.07':
    full_name: 'CCleaner 5.07'
    installer: 'http://download.piriform.com/ccsetup507.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.06':
    full_name: 'CCleaner 5.06'
    installer: 'http://download.piriform.com/ccsetup506.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
  '5.02.5101':
    full_name: 'CCleaner 5.02'
    installer: 'http://download.piriform.com/ccsetup502.exe'
    install_flags: '/S'
    uninstaller: '%ProgramFiles(x86)%\CCleaner\uninst.exe'
    uninstall_flags: '/S'
    msiexec: False
    locale: en_US
    reboot: False
